...
The "HTTP API v2" should be enabled; without 1P/3P phase switching, the "HTTP API v1" is sufficient as well. Both can be set in the go-e app ("Settings" > "Allow access to local HTTP API v2" and further down for "v1"). With the current firmware, "v1" is obsolete if not used otherwise.
